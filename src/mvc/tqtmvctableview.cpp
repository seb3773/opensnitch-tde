...
void TQtMvcTableView::rebuildIndex()
{
    ...
    if (m_sortColumn >= 0 && m_sortColumn < cols && (int)m_visibleRows.size() > 1) {
        ...
        // Rebuild m_visibleRows from sorted indices
        TQValueVector<int> sorted(n);
        for (int i = 0; i < n; ++i)
            sorted[i] = m_visibleRows[sortIdx[i]];
        m_visibleRows = sorted;
    }

    // --- Step 3: sync table widget ---
    rebuildReverseIndex();
    setNumRows((int)m_visibleRows.size());
}
                                ...
                                const TQRect& cr, bool selected)
{
    ...
    if (selected) {
        ...
        bgColor = (row % 2 == 0) ? cg.base() : cg.base().dark(105);
    }
...
void TQtMvcTableView::onHeaderClicked(int col)
{
    ...
    // Update header labels with simple ASCII sort indicator
    int cols = m_model->columnCount();
    for (int c = 0; c < cols; ++c) {
        TQString label = m_model->headerData(c);
        if (c == m_sortColumn) {
            label += m_sortAscending ? " [+]" : " [-]";
        }
        horizontalHeader()->setLabel(c, label);
    }
