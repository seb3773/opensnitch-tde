...
# ==============================================================================
# OpenSnitch TDE - Q4OS Installer (.qsi) Builder
# ==============================================================================
...
echo "=================================================="
echo " OpenSnitch TDE - Q4OS .qsi Package Builder"
echo "=================================================="
...
find_latest_deb() {
    local debs=("$SCRIPT_DIR"/opensnitch-tde_[0-9]*_*.deb "$SCRIPT_DIR"/*.deb "$SCRIPT_DIR"/build/*.deb)
    for f in "${debs[@]}"; do
        if [ -f "$f" ]; then
            echo "$f"
            return 0
...
PACKAGE_NAME=$(dpkg-deb -f "$LATEST_DEB" Package 2>/dev/null || echo "opensnitch-tde")
APP_VERSION=$(dpkg-deb -f "$LATEST_DEB" Version 2>/dev/null || echo "1.5.8")
...
chmod +x "$TEMPLATES_DIR"/*.dvt 2>/dev/null || true
...
APPNAME_DESC="OpenSnitch Firewall UI for Trinity"
APP_ICON="opensnitchtde"
PK_VERS="$APP_VERSION"
SETUP_TYPE="2"
